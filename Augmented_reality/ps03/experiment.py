...
def helper_for_part_4_and_5(video_name, fps, frame_ids, output_prefix,
    ...
    frame_num = 1
    rotated_templates = ps3.finer_rotated_templates()
    ...
    while image is not None:
        ...
        markers = ps3.find_markers_quarters(image, rotated_templates)
        ...
        if is_part5:
            try:
                while num_of_trial <= 2:
                    homography = ps3.find_four_point_transform_1(src_points, markers)

                    image = ps3.project_imageA_onto_imageB_beyond_inverse_warping(advert, image,
                                                                                  homography)
                    if image is not None:
                        prev_homography = homography
                        prev_image = orig_image
                        break

                    print("Trying with previous homography")
                    num_of_trial = num_of_trial + 1

                # for marker in markers:
                #     mark_location(image, marker)
            except:
                print("Exception calculating homography")
                image = image_gen.__next__()
                frame_num += 1
                continue
        else:
...
def helper_for_part_7(input_video, input_image, output_video, output_prefix, frame_ids, fps, aruco_dict):
    ...
    while True:
        ...
        try:
            ...
            print("Exception while getting acuro")
            markers, ids = prev
        if len(markers) == 4:
            # find the centers of four ArUco markers
            save_image("frame.jpg", frame)
            centers = ps3.find_aruco_center(markers, ids)
            id_center_list = list(zip(ids.reshape(4).tolist(), centers))
            id_center_list.sort(key=lambda x: x[0])
            centers = list(map(lambda x: x[1], id_center_list))
            # project the input image onto a wall image with ArUco markers
            homography = ps3.find_four_point_transform_1(src_points, centers)
            frame = ps3.project_imageA_onto_imageB_beyond_inverse_warping(image, frame, homography)

            # mark the center and place an ID string of each ArUco marker
            for id, center in id_center_list:
...
def part_2():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        # Open image and identify the four marker positions
        scene = cv2.imread(os.path.join(IMG_DIR, img_in))
        rotated_templates = ps3.rotated_templates()
        markers = ps3.find_markers_quarters(scene, rotated_templates)
...
def part_2_test():
    ...
    output_images = ['ps3-5-a-2_out.png']
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        # Open image and identify the four marker positions
        scene = cv2.imread(os.path.join("{}".format(IMG_DIR), img_in))
        rotated_templates = ps3.finer_rotated_templates()
        markers = ps3.find_markers_quarters(scene, rotated_templates)
        image_with_box = ps3.draw_box(scene, markers, 3)
...
def part_3():
    ...
    # Optional template image
    template = cv2.imread(os.path.join(IMG_DIR, "template_proc.jpg"))
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        scene = cv2.imread(os.path.join(IMG_DIR, img_in))
        rotated_templates = ps3.finer_rotated_templates()
        markers = ps3.find_markers_half(scene, rotated_templates)
        markers = ps3.sort_by_leftright(markers)

        homography = ps3.find_four_point_transform(src_points, markers)

        projected_img = ps3.project_imageA_onto_imageB_beyond_inverse_warping(advert, scene,
                                                                              homography)
        # for marker in markers:
        #     mark_location(scene,marker)
...
def helper_for_part_6(video_name, my_ad_name, fps, frame_ids, output_prefix,
                      counter_init):
    video = os.path.join(VID_DIR, video_name)
    ad_video = os.path.join(VID_DIR, my_ad_name)

    image_gen = ps3.video_frame_generator(video)
    ad_generator = ps3.video_frame_generator(ad_video)

    image = image_gen.__next__()
    advert = ad_generator.__next__()
    ...
    frame_num = 1
    rotated_templates = ps3.rotated_templates()
    while image is not None and advert is not None:
        orig_image = np.copy(image)
        src_points = ps3.get_corners_list(advert)
        ...
        try:
            homography = ps3.find_four_point_transform_1(src_points, markers)
            image = ps3.project_imageA_onto_imageB_beyond_inverse_warping(advert, image, homography)
        except:
            print("Exception calculating homography")
            image = image_gen.__next__()
            advert = ad_generator.__next__()
        ...
        if frame_num in frame_ids:
            out_str = output_prefix + "-{}.png".format(output_counter)
            save_image(out_str, image)
            # save_image("test.jpg", orig_image)
            output_counter += 1
...
def part_6():
    ...
    frame_ids = [355, 555, 725]
    fps = 40
    helper_for_part_6(video_file, my_video, fps, frame_ids, 'ps3-6-a-1', 1)
