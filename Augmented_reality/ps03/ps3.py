...
def find_markers_half(temp, rotated_templates):
    temp_img = cv2.imread("input_images/template_proc.jpg")

    temp = denoise(temp)
    gray = cv.cvtColor(temp, cv.COLOR_BGR2GRAY)
    _, gray = cv.threshold(gray, 75, 255, cv.THRESH_BINARY_INV)
    # cv.imshow("win",gray)
    # cv.waitKey(0)
    x = gray.shape[1]
    y = gray.shape[0]
    halfs = [(gray[0:int(y), 0: int(x / 2)], 0, 0),
                (gray[0:int(y), int(x / 2):x], int(x / 2), 0)]
    save_image("half.jpg", halfs[1][0])
    markers = []
    for half, x, y in halfs:
        results = process_perhalf(half, rotated_templates)
        for x_cord,y_cord in results:
...
def find_markers_quarters(temp, rotated_templates):
    ...
    # cv.imshow("Quarters", quarters[0][0])
    # cv.waitKey(0)
    markers = []
...
def process_perhalf(quarter, rotated_images):
    results = {}
    for rotated_image in rotated_images:
        ...
        if loc is not None and len(loc[::-1][0]) > 2:
            # print(len(loc[::-1][0]))
            x = []
            y = []
            for pt in zip(*loc[::-1]):
...
def sort_by_leftright(marker):
    sortby_x = sorted(marker, key=lambda point: point[0])

    first_half = sortby_x[:2]
    second_half = sortby_x[2:4]

    first_half_sort_by_y = sorted(first_half, key=lambda point:point[1])
    second_half_sort_by_y = sorted(second_half, key=lambda point:point[1])

    return [first_half_sort_by_y[0], first_half_sort_by_y[1], second_half_sort_by_y[0], second_half_sort_by_y[1]]
...
def project_imageA_onto_imageB_beyond_inverse_warping(advert, scene, homography):
    """Projects image A into the marked area in imageB.
    ...
    dst = cv2.remap(advert, map_x, map_y, cv2.INTER_NEAREST)

    scene = cv2.addWeighted(scene, 0.5, dst, 0.5, -1)
