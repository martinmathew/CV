...
cv2.createTrackbar(threshold1, window, 630, 5000, nothing)
cv2.createTrackbar(threshold2, window, 200, 5000, nothing)
...
cv2.createTrackbar(ke, window, 2, 100, nothing)
# img = ps2.denoise(img)
while 1:
    temp = img.copy()
    ...
    # threshold = 80
    # gray = cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY)
    # canny_output = cv2.Canny(temp, threshold1, threshold2, apertureSize=3)
    ...
    #     drawing = cv2.erode(drawing, (13,13),iterations=1)
    #     circles = cv2.HoughCircles(drawing, cv2.HOUGH_GRADIENT, dp=dp, minDist=minDist,
    #                                param1=param1, param2=param2, minRadius=minRadius, maxRadius=maxRadius)
    #
