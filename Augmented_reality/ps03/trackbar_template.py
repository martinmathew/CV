...
while 1:
    inp_img_files = {'ps3-3-c_base.jpg': [(109, 80), (119, 252), (499, 40), (503, 277)]}
    ...
    for angle in range(0, 180, 15):
        ...
        rotated_template[angle] = rotated_img.copy()

    found = False
    for threshold in range(1, 100):
        for gray_scale_threshold in range(91, 255):
            ...
            for img_file in inp_img_files.keys():
                ...
                for angle in rotated_template.keys():
                    print("Angle - {}".format(angle))
                    rotated_img = rotated_template[angle]
                    # cv2.imshow("Test11", rotated_img)
                    res = cv2.matchTemplate(gray, rotated_img, cv2.TM_CCOEFF_NORMED)

                    loc = (np.vstack(np.unravel_index(np.argpartition(res.flatten(), -10)[-10:], res.shape))[0],
                           np.vstack(np.unravel_index(np.argpartition(res.flatten(), -10)[-10:], res.shape))[1])
                    max_tenth = np.sort(res.flatten())[::-10][0]
                    ...
                    if loc is not None and len(loc[::-1][0]) >= 4:
                        ...
                        if len(dict) < 4:
                            ...
                            continue
                        # print("Four Corners Founds for - {}".format(img_file))
