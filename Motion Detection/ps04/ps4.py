...
def optic_flow_lk(img_a, img_b, k_size, k_type, sigma=1):
    ...
    Iy = cv2.Sobel(img_b, cv2.CV_64F, 0, 1, ksize=3, scale=1 / 8)
    It = img_a - img_b
    ...
    au = (Syy * Sxt - Sxy * Syt)
    av = (-Sxy * Sxt + Sxx * Syt)
    U = np.divide(au, b, out=np.zeros_like(au), where=b!=0)
    V = np.divide(av, b, out=np.zeros_like(av), where=b!=0)
    U[np.isnan(U)] = 0
    V[np.isnan(V)] = 0
    return U, V
...
def optic_flow_lk1(img1, img2, k_size, k_type, windows, sigma=1):
    ...
    SIxx = cv2.filter2D((Ix * Ix), -1, kernel)
    SIxy = cv2.filter2D((Ix * Iy), -1, kernel)
    SIyy = cv2.filter2D((Iy * Iy), -1, kernel)

    SIxt = cv2.filter2D((Ix * It), -1, kernel)
    SIyt = cv2.filter2D((Iy * It), -1, kernel)

    Ata = np.array([[SIxx, SIxy], [SIxy, SIyy]])
    Ata = Ata.reshape(Ata.shape[2], 2, 2)

    b = np.array([-SIxt, -SIyt])
    b = b.reshape(b.shape[1], 2)
    uv = np.matmul(np.linalg.pinv(Ata), b)
...
def hierarchical_lk(img_a, img_b, levels, k_size, k_type, sigma, interpolation,
    ...
    for i in range(levels - 1, -1, -1):
        ...
        ugi, vgi = optic_flow_lk(gpyr1[i], warp_img, k_size, k_type, sigma)
        uk1 = uk1 + ugi
        vk1 = vk1 + vgi
