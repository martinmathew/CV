...
def copy_paste_middle(src, dst, shape):
    ...
    dst[int(destleftRP[0]):int(destrightBT[0]),int(destleftRP[1]):int(destrightBT[1])] = src[int(srcleftRP[0]):int(srcrightBT[0]), int(srcleftRP[1]):int(srcrightBT[1])]
    return dst
...
def image_stats(image):
    return (np.max(image), np.min(image), np.mean(image), np.std(image))
...
def center_and_normalize(image, scale):
    return ((image - np.mean(image))/np.std(image))*scale*10 +  np.mean(image)
...
def shift_image_left(image, shift):
    ...
    shifted = cv.warpAffine(image, transition_matrix, (image.shape[1], image.shape[0]))
    cv.copyMakeBorder(shifted, 0, 0, shift, 0, cv.BORDER_REPLICATE)
    return shifted
...
def add_noise(image, channel, sigma):
    rand_mat = np.random.rand(image.shape[0], image.shape[1]) * sigma
    image[:, :, channel] = image[:, :, channel] + rand_mat
    return image
